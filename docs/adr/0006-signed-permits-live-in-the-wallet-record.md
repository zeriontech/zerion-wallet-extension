...
> **Superseded by [ADR-0007](./0007-signed-permits-live-in-storage-session.md)** (2026-09-16). Permits were never actually written to disk (`walletToObject` did not copy the field), so the encryption-at-rest argument below did not hold, and keeping an in-memory copy on the record meant it was lost on every service worker restart. The field `confidentialPermits` no longer exists on `ExternallyOwnedAccount`. Kept for the reasoning history.
