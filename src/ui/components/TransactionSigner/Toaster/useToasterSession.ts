...
export interface ActiveStepView {
  toaster?: ToasterView;
  /** Broadcast transaction hash, once the step has one */
  hash: string | null;
}
...
export function useToasterSession(): ToasterSession {
  ...
  useEffect(() => {
    function getStepView(queueId: string, stepIndex: number): ActiveStepView {
      ...
      return { toaster: step?.toaster, hash: null };
    }

    /**
     * Attach the step's broadcast hash to the view already on screen.
     * Guarded by queueId so a late event from a finished queue can't relabel
     * the step the toaster is currently showing.
     */
    function setStepHash(queueId: string, hash: string) {
      setState((s) => {
        if (!s.current || s.activeQueueId !== queueId) return s;
        return { ...s, current: { ...s.current, hash } };
      });
    }
      ...
      (queueId: string, event: QueueEvent) => {
        ...
        switch (event.type) {
          ...
          case 'step-pending': {
            ...
            setStepHash(queueId, event.txHash);
            break;
          }
          case 'step-success': {
