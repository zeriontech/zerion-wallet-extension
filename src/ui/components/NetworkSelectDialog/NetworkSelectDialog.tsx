          ...
          {groups.map((group, index) =>
            group.items.length ? (
              <NetworkList
                ...
                chainDistribution={chainDistribution}
                showAllNetworks={showAllNetworks && index === 0}
...
function SearchView({
  ...
  const items = useMemo(() => {
    const allNetworks =
      groups?.flatMap((group) => group.items) || showTestnets
        ? networks?.getAllNetworks()
        : networks?.getMainnets();
    return allNetworks?.filter(filterNetworksByQuery(query));
  }, [groups, query, networks, showTestnets]);
  if (isLoading) {
...
function AddressNetworkList({
  ...
  const groups = useMemo(() => {
    return (
      maybeGroups || createGroups({ networks, chainDistribution, showTestnets })
    );
  }, [maybeGroups, networks, chainDistribution, showTestnets]);
