...
function PerpsHistoryList({ address }: { address: string }) {
  ...
  if (isInitialLoading) {
    return <PerpsHistoryListSkeleton />;
  }
...
export function PerpsOverview() {
  ...
  // Lifted here as the single source for the positions list (presentational
  // below), the markets-list loading gate, and the DEX list. React Query
  // dedupes, so the now-presentational PerpsPositionsList no longer fetches.
  const {
    allPositions,
  ...
  return (
    <VStack gap={16}>
      ...
      <PerpsPositionsList
        ...
        isLoading={positionsLoading}
        coinToMarkPx={coinToMarkPx}
      />
      <PerpsMarketsList dexList={dexList} positionsLoading={positionsLoading} />
      <Spacer height={16} />
      <Dialog2
