...
function DepositFormBody({
  ...
  const { mutate: handleDeposit, ...signMutation } = useMutation({
    mutationFn: async () => {
      ...
      // Both Approve and Bridge steps share the same toaster view so the user
      // sees a single continuous "Bridging" pill across the two-tx flow — the
      // approve step is an implementation detail and shouldn't surface as a
    ...
    onSuccess: () => {
      ...
      // alone. Keyed flat `[name, address, dexIdentifier]`, so partial-match
      // needs predicate form.
