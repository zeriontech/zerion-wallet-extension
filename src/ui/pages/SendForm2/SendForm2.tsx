...
function SendFormComponent({
  ...
  const sendTransaction = sendData?.transaction ?? null;
  const sendNetworkFee = sendData?.networkFee ?? null;
  ...
  const { mutate: handleSignTransaction, ...signMutation } = useMutation({
    mutationFn: async (result: SimulationResult) => {
      ...
      invariant(sendData?.transaction, 'Send transaction must be prepared');
      ...
      // Backend-prepared EVM tx comes back with a nonce that's stale once
      // another queue is in flight. Strip it so prepareNonce recomputes
      // against the freshest local+RPC state at sign time. Exception: a
      // user-entered nonce in form state overrides.
      const transaction = sendData.transaction;
      const finalTransaction = transaction.evm
        ? userNonce != null
          ? { evm: { ...transaction.evm, nonce: userNonce } }
          : (() => {
              const { nonce: _omit, ...rest } = transaction.evm;
              return { evm: rest as typeof transaction.evm };
            })()
        : transaction;

      const feeValueCommon = sendData.networkFee?.amount?.quantity ?? null;
      ...
      const steps: SignStep[] = [
        {
          ...
          toaster: {
            ...
            recipient: { address: formState.to },
            isNft: isNftMode,
          },
        },
      ];
  ...
  const evmTx = sendData?.transaction?.evm ?? null;
