...
export class GhostWallet implements Wallet {
  ...
  constructor(implementation: Ghost) {
    ...
    implementation.on('connect', this.#connected, this);
    implementation.on('disconnect', this.#disconnected, this);
    implementation.on('accountChanged', this.#reconnected, this);
  ...
  #connected = () => {
    const address = this.#implementation.publicKey?.toBase58();
    if (address) {
      const publicKey = this.#implementation.publicKey!.toBytes();
  ...
  #reconnected = () => {
    if (this.#implementation.publicKey) {
      this.#connected();
    } else {
      this.#disconnected();
    }
  ...
  #signTransaction: SolanaSignTransactionMethod = async (...inputs) => {
    ...
    if (inputs.length === 1) {
      ...
      const signedTransactions = await this.#implementation.signAllTransactions(
        transactions
      );
