...
function trackAppEvents({ account }: { account: Account }) {
  ...
  const trackTransactionSent = async (
    props: {
      context: { mode: 'default' | 'testnet' } & TransactionContextParams;
    } & (
      | { status: 'success'; result: SignTransactionResult }
      | {
          status: 'failed';
          errorMessage: string;
        }
  ...
  emitter.on('transactionSent', (result, context) =>
    trackTransactionSent({ status: 'success', result, context })
  );

  emitter.on('transactionFailed', (errorMessage, context) =>
    trackTransactionSent({ status: 'failed', errorMessage, context })
  );
